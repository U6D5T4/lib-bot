        ...
        switch (callbackQuery.Data.ToLower())
        {
            ...
            case "no":
                if (data.ChatState == ChatState.AllBooks || data.ChatState == ChatState.SearchBooks)
                {
                    ...
                    var message = data.ChatState == ChatState.AllBooks ? _resourceReader.GetString("BooksLibrary") + $"{Environment.NewLine}" + GetFiltersAsAStringMessage(data.Filters) : _resourceReader.GetString("SearchQueryResult");
